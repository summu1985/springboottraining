...
public class ProductController {
	...
	public String products(Model model) {
		ResponseEntity<Product[]> response = restTemplate.getForEntity("http://localhost:8080/store/api/products/",
				Product[].class);
	...
	public String doDeleteProduct(@RequestParam(value = "id", required = true) String id, Model model) {
		...
		String deleteURL = "http://localhost:8080/store/api/product/" + id + "/";
		String response;
		try {
	...
	public String doShowproduct(@RequestParam(value = "id", required = true) String id, Model model) {
		...
		ResponseEntity<Product> response = restTemplate
				.getForEntity("http://localhost:8080/store/api/products/" + id + "/", Product.class);
