...
public class ProductAPIController {
	...
	public ResponseEntity updateProductById(@PathVariable(value = "id") int id, @RequestBody Product product) {
		Optional<Product> updatedProduct = prodService.updateProduct(id, product);
		if (updatedProduct.isPresent()) {
			return new ResponseEntity("Product id " + id + " updated successfully.", HttpStatus.OK);
		} else {
			return new ResponseEntity("No product found with product id: " + id, HttpStatus.NOT_FOUND);
		}
